...
object SettingsStatsCollector {
    ...
    fun logNewSettings(configurable: DeepBugsConfig, ui: DeepBugsUI) {
        generateConfigEventData(configurable, ui).forEach { data ->
            DeepBugsCounterLogger.logEvent(SETTINGS, CONFIG_THRESHOLD, data)
        }
    }

    private fun generateConfigEventData(config: DeepBugsConfig, ui: DeepBugsUI): List<FeatureUsageData> {
    ...
    private fun generateConfigData(
        ...
        config: DeepBugsConfig,
        ui: DeepBugsUI
    ): FeatureUsageData {
        val (prevConfig: Float, newConfig: Float) = when (inspectionClass) {
            "operator" -> Pair(config.state.binOperatorThreshold, ui.binOperatorThreshold)
            "operand" -> Pair(config.state.binOperandThreshold, ui.binOperandThreshold)
            "call" -> Pair(config.state.swappedArgsThreshold, ui.swappedArgsThreshold)
