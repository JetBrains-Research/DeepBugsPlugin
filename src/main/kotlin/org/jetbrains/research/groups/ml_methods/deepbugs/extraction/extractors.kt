    ...
    is PyPrefixExpression -> {
        ...
        if (node.operator == PyTokenTypes.MINUS) {
            ...
            if (operand is PyNumericLiteralExpression) {
                val intValue = -operand.text.toDouble()
                value = intValue.toString().asLiteralString()
            }
        }
        value
    }
...
fun extractPyNodeBase(node: PyElement?): String = when (node) {
    is PyCallExpression -> node.firstChild.firstChild.reference?.element?.text?.asIdentifierString() ?: ""
